...
Git UI Pro 是一款中文桌面 Git 可视化管理软件。产品核心不是替代命令行 Git，而是通过图形界面帮助用户查看项目历史、理解分支关系、处理常见 Git 操作，并管理多个本地或 SSH 远程 Git 项目。
...
- 支持多项目添加、搜索、切换和基础分组。
- 支持通过系统 OpenSSH 直接打开服务器上的 Git 工作区，不要求本地克隆。
...
- MVP 不做完整 remote 配置管理，remote 地址管理先只读。
...
- 本地项目使用用户本机 Git 配置、SSH key、Git Credential Manager 和 credential helper。
- SSH 项目使用系统 OpenSSH、SSH Agent、`~/.ssh/config` 或用户指定的私钥；应用不保存 SSH 密码。
...
1. 用户点击“连接远程 Git 项目”。
2. 输入 SSH 主机或配置别名、可选用户名和端口、服务器仓库绝对路径，并可指定私钥文件。
3. 应用通过系统 OpenSSH 在服务器执行 `git -C <path> rev-parse --show-toplevel`。
4. 连接与仓库校验通过后保存项目记录，并直接加载服务器工作区状态和历史。
5. 打开控制台时，通过 SSH 进入同一服务器仓库目录。

认证边界：

- 支持 SSH Agent、`~/.ssh/config`、默认私钥或指定私钥。
- Git 后台操作使用非交互认证，不弹出或保存 SSH 密码。
- 首次主机指纹确认、带口令私钥解锁应先在系统终端或 SSH Agent 中完成。
- 已记录的主机指纹发生变化时必须阻断连接，不自动忽略校验。
