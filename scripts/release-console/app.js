...
function getNotes() {
  return elements.releaseNotes.value
    .split(/\r?\n/)
...
function updateNoteCount() {
  ...
  setText(elements.noteCount, `${count} 条`);
  elements.noteCount.classList.toggle("invalid", count > 12);
...
function renderStatus(status, preserveForm = false) {
  ...
  if (!preserveForm) {
    ...
    if (!notesWereEdited) {
      elements.releaseNotes.value = status.suggestedNotes.join("\n");
    }
  }
  validateVersion();
  updateNoteCount();
...
elements.releaseNotes.addEventListener("input", () => {
